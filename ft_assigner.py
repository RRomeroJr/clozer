...
chpnt = find_checkpoint(chpnts_dir=chpnts_dir)
...
ITERS = 1
EPOCHS = 2 # something even good to not make batch size weird
BATCH_SIZE = 2
...
origdataset = load_dataset('parquet', 
                    data_files={
                        'train': 'datasets/parquet/deck_assign_train.parquet',
                        'test': 'datasets/parquet/deck_assign_test.parquet'
                    },
                    split=None)
...
for i in range(ITERS):
    ...
    trainer = UnslothTrainer(
        ...
        args = UnslothTrainingArguments(
            ...
            logging_steps = 4, #only matters is logging_strategy="steps"
            optim = "adamw_8bit",
            weight_decay = 0.01,
            lr_scheduler_type = "linear",
            seed = 3407,
            output_dir = chpnts_dir,
    ...
    chpnt = find_checkpoint(silent = True, chpnts_dir=chpnts_dir)
