...
r'nY>J|^.mER', r'B?oy0[Ei7a', r'CsY-u+GuGX', r'rGuzVG8ATv', r'mLXLI8zb(m', r'Awa`=(XuN@', r'B8^Glj#[Gd', r'Od=[-jz<mz', r'us~.xJ~0DM', r'x;2|aArNF.', r'I,;&xrUtVm', r'eg4DeS8|Z3', r'uUASR;qdjc', r'kQQvQZ{ct-', r'l.u{[ebo>q', r'Elkr,}M[sd', r'e>SR;+A5y&'}
    # test_guids = set()
    if acr.test_guids == None:
        acr.test_guids = _test_guids_v1
    if False and acr.test_guids == None: # get assign some guids as train
        for count, row in enumerate(acr, start=1):
            if any(omitted_deck in row.deck for omitted_deck in omitted_decks):
                print(f"test builder found {row.deck}, skipping note")
                continue
        ...
        for deck, guids in deck_to_guids.items():
            if len(guids) > 2:
                acr.test_guids.update(random.sample(guids, 2))
                continue
            elif len(guids) == 2: # 1 for train 1 for test.
                acr.test_guids.add(random.choice(guids))
                continue
            # else: then we 1 or none. If we have 1 we can't use it to test.
