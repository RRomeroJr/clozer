...
# SYSTEM_MSG_OBJ = None
SYSTEM_MSG_OBJ = MsgObj("system", finetune_sys_prompt2)
def scroll():
...
def main():
    ...
    path = 'datasets/anki_exports/training_set4.csv'
        ...
        for ufs in (unique_files_train, unique_files_test):
            ...
            for fn, note_objs in ufs.items():
                ...
                for count, ns in enumerate(note_split, start=1):
                    ...
                    src_memo: Dict[str, List[str]] = {}
                    for r_i, r_obj in enumerate(resp_objs):
                        src_memo.setdefault(r_obj["src"], []).append(r_i)
                        del r_obj["src"]
                    
                    
                    exchange = MsgExchange(
                        MsgObj("user", ffile_str),
                        MsgObj("assistant", {"anki_notes": resp_objs, "sources": src_memo}),
                        system = SYSTEM_MSG_OBJ)
